...
var translateWord = function (word, options) {
	...
	var isAllUppercase = function (word){
		return /^\p{Lu}+$/u.test( word );
	}
    ...
    if (!(!options.showall && ignored(word))) {
        if (show_different) {
            if (different_meanings[word]) {
                if ((!options.american && different_meanings[word]["British English"]) || (!options.british && different_meanings[word]["American English"])) {
                    return addToOutput(originalWord, "Different meanings", different_meanings[word]);
                }
            }
        }
        ...
        if (show_spelling) {
            ...
            if (!options.american && american_spelling) {
                return addToOutput(originalWord, "British English Spelling", american_spelling);
            } else {
                var british_spelling = american_spellings[word];
                if (!options.british && british_spelling) {

					if ( isUppercase( originalWord ) ) {
						british_spelling = british_spelling.charAt(0).toUpperCase() + british_spelling.slice(1);
					}

					if ( isAllUppercase( originalWord ) ) {
						british_spelling = british_spelling.toUpperCase();
					}

                    return addToOutput(originalWord, "American English Spelling", british_spelling);
                }
            }
        }
        ...
        if (show_exclusive) {
            if (!options.british && (word in american_only)) {
                return addToOutput(originalWord, "American English", american_only[word]);
            }

            if (!options.american && (word in british_only)) {
                return addToOutput(originalWord, "British English", british_only[word]);
            }
