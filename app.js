...
function translateCSV(file) {
	fs.createReadStream(path.resolve(__dirname, file))
		...
		.transform((row, next) => {
			...
			let translations = translateWords(
				stripper.stripHtml(row.word_from).result
			);
				...
				for (let property in translations) {
					translations[property].forEach(function (translation) {
						for (let word in translation) {
							console.log(
								word + ' => ' + translation[word].details
							);
							translatedWords = translatedWords.replaceAll(
								word,
								translation[word].details
							);
						}
					});
				}
		...
		.pipe(
			fs.createWriteStream(
				path.resolve(__dirname, file.replace('.csv', '-translated.csv'))
			)
		)
		.on('end', () => process.exit());
}
